...
import {
  RiDashboardLine, RiCalendarLine, RiHeartLine,
  RiWalletLine, RiUserLine, RiSettings3Line,
  RiLogoutBoxLine, RiMenuFoldLine, RiMenuUnfoldLine,
  RiStoreLine, RiStarLine, RiBarChart2Line,
  RiShieldUserLine, RiGroupLine, RiFileListLine,
  RiAlertLine, RiArrowRightSLine,
} from "react-icons/ri";
...
const NAV_SETS = {
  user: [
    ...
    { icon: RiHeartLine,     label: "Wishlist",     path: "/dashboard/wishlist"  },
    { icon: RiWalletLine,    label: "Budget",       path: "/budget"              },
  ...
  admin: [
    ...
    { icon: RiAlertLine,     label: "Disputes",     path: "/admin/disputes", badge: "3" },
    { icon: RiShieldUserLine,label: "Approvals",    path: "/admin/approvals", badge: "38" },
    { icon: RiSettings3Line, label: "Settings",     path: "/admin/settings"      },
  ],
};

export default function DashboardSidebar({ role = "user", user = {}, collapsed, setCollapsed }) {
  const links = NAV_SETS[role] || NAV_SETS.user;

  const roleLabel = { user: "Couple", vendor: "Vendor", admin: "Admin" }[role];
  const roleColor = { user: "text-gold", vendor: "text-maroon", admin: "text-maroon" }[role];
    ...
    <>
      ...
      >
        ...
        <div className={`flex items-center h-16 px-4 border-b border-white/8
          ...
          {!collapsed && (
            <Link to="/" className="font-heading text-xl font-semibold text-ivory">
              Wed<span className="text-gold">L</span>
            </Link>
          )}
          <button
        ...
        <nav className="flex-1 overflow-y-auto py-3 hide-scrollbar">
          {links.map(({ icon: Icon, label, path, badge }) => (
            ...
            >
              ...
              {!collapsed && badge && (
                <span className="ml-auto text-[10px] font-bold bg-gold text-dark
              ...
              {collapsed && (
                <div className="absolute left-full ml-2 px-2.5 py-1.5 rounded-lg
                  ...
                  {label}
                  {badge && <span className="ml-1.5 text-gold font-bold">({badge})</span>}
                </div>
              )}
            </NavLink>
          ))}
        ...
        <div className="flex items-center justify-around px-2 py-2">
          {links.slice(0, 5).map(({ icon: Icon, label, path, badge }) => (
            ...
            >
              ...
              <span>{label}</span>
              {badge && (
                <span className="absolute -top-0.5 right-2 text-[8px] font-bold
