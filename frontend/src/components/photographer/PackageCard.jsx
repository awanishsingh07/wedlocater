  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col rounded-luxury border shadow-luxury
                  hover:shadow-luxury-md hover:-translate-y-1 transition-all duration-300 overflow-hidden
                  ${popular ? "bg-maroon-gradient border-maroon/20" : "bg-white border-cream"}`}
    >
      {popular && (
        <div className="absolute top-0 left-0 right-0 flex justify-center">
          <span className="inline-flex items-center gap-1 px-4 py-1 bg-gold text-dark
                           text-[10px] font-body font-bold rounded-b-xl shadow-gold">
            <RiSparklingLine size={10} /> Most Popular
          </span>
        </div>
      )}
      <div className={`flex flex-col flex-1 p-5 sm:p-6 ${popular ? "pt-8" : ""}`}>
        <p className={`font-body text-xs font-semibold uppercase tracking-widest mb-2
                       ${popular ? "text-gold/80" : "text-muted"}`}>{name}</p>
        <span className={`font-heading text-4xl font-semibold leading-none mb-0.5
                          ${popular ? "text-ivory" : "text-maroon"}`}>{FMT(price)}</span>
        {duration && (
          <p className={`font-body text-xs mb-5 ${popular ? "text-ivory/50" : "text-muted"}`}>{duration}</p>
        )}
        ...
        <ul className="flex flex-col gap-2.5 flex-1 mb-6">
          {features.map((f, i) => (
            <li key={i} className="flex items-start gap-2.5">
              ...
              <span className={`font-body text-xs leading-snug ${popular ? "text-ivory/75" : "text-muted"}`}>{f}</span>
            </li>
          ))}
        </ul>
        <button onClick={() => onSelect && onSelect(pkg)}
